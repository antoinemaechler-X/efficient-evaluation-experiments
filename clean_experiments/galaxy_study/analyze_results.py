...
cross_ppi_files = glob.glob("results/cross_ppi_sl=*.csv")
...
dfs = [
    pd.concat([pd.read_csv(f) for f in wor_files], ignore_index=True),
    pd.concat([pd.read_csv(f) for f in bernoulli_files], ignore_index=True),
    pd.concat([pd.read_csv(f) for f in cross_ppi_files], ignore_index=True),
]
# Active inference is optional (pending); include it if the files are there.
if active_inf_files:
