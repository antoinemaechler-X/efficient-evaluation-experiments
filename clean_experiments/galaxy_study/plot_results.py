...
# active-inference is pending; it shows up automatically once its results exist.
method_config = [
...
def make_figure(ess_col, ess_serr_col, methods, out_path, legend_ncol=3, budget_subset=None):
    ...
    """
    # Only show methods that actually have results (skips e.g. active-inference while pending).
    ...
    for group_name, title, col in groups_config:
        ...
        for method, marker, color, label, annotate in methods:
            ...
            if q.empty:  # method not run yet (e.g. active-inference still pending)
                continue
...
make_figure("ess_multiplier", "ess_multiplier_serr",
            method_config,
            "results/galaxy_ess+coverage_vs_wor_uniform.pdf")

# Plot 2: All 6 methods, ESS relative to Classical
make_figure("ess_multiplier_vs_classical", "ess_multiplier_vs_classical_serr",
            method_config,
            "results/galaxy_ess+coverage_vs_classical.pdf")

# Plot 3: WOR Active vs Active Inference vs Cross-PPI vs Classical, ESS relative to Classical.
# active-inference is pending, so for now this figure shows WOR / Cross-PPI / Classical;
# the Active Inference line fills in once run_active_inference.py has been run.
method_config_main = [
    ("wor-active",        "o", colors[0], "WOR Active (Ours)",        True),
...
make_figure("ess_multiplier_vs_classical", "ess_multiplier_vs_classical_serr",
            method_config_main,
            "results/galaxy_main_sparse.pdf",
