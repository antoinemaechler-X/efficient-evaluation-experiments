"""
Plot Amazon deforestation study results.

Produces PDFs:
  1. Main: WOR Active vs Cross-PPI vs Active Inference vs Classical (ESS + coverage)
  2. All methods: all 7 methods vs WOR-uniform
  3. All methods: all 7 methods vs Classical
...
# --- Load data ---
data_dir = "data"
Y = np.load(f"{data_dir}/Y_test.npy").flatten()
...
if not os.path.exists("results/summary.csv"):
    print("results/summary.csv not found. Run analyze_results.py first (after the experiments).")
    sys.exit(0)
...
groups_config = [
    ("all", "Amazon Deforestation - Fraction Deforested", 0),
]
...
def make_figure(ess_col, ess_serr_col, methods, out_path, legend_ncol=3, budget_subset=None):
    ...
    """
    # Only show methods that actually have results (all four are pending at first).
...
# Plot 1: ESS relative to WOR-uniform. Only meaningful once WOR has run, so skip it otherwise.
if "wor-uniform" in summary["method"].unique():
    make_figure("ess_multiplier", "ess_multiplier_serr",
                method_config,
                "results/deforestation_ess+coverage_vs_wor_uniform.pdf")

# Plot 2: All 7 methods, ESS relative to Classical
make_figure("ess_multiplier_vs_classical", "ess_multiplier_vs_classical_serr",
            method_config,
            "results/deforestation_ess+coverage_vs_classical.pdf")

# Plot 3: WOR Active vs Cross-PPI vs Active Inference vs Classical, ESS relative to Classical
method_config_main = [
    ("wor-active",         "o", colors[0], "WOR Active (Ours)",        True),
    ("cross-ppi",          "P", colors[5], "Cross-PPI (Zrnic)",        True),
    ("active-inference",   "*", colors[7], "Active Inference (Zrnic)", True),
    ("bernoulli-active",   "s", colors[3], "Bernoulli Active",         True),
    ("classical",          "^", colors[2], "Classical",                 False),
]
make_figure("ess_multiplier_vs_classical", "ess_multiplier_vs_classical_serr",
            method_config_main,
            "results/deforestation_main.pdf",
            legend_ncol=3)
...
make_figure("ess_multiplier_vs_classical", "ess_multiplier_vs_classical_serr",
            method_config_main,
            "results/deforestation_main_sparse.pdf",
            legend_ncol=3, budget_subset=budgets_10)
