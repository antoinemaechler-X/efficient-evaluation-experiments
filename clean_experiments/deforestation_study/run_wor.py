"""
WOR estimator for Amazon deforestation dataset.

Usage: python run_wor.py <seed_chunk>
  seed_chunk in {0, 1, 2}: splits 1000 seeds into 3 chunks.

Runs WOR-active and WOR-uniform on the test population (no groups).
Uses fixed HistGradientBoostingClassifier predictions from download_data.py.
Scoring: min(Yhat, 1-Yhat) for active, uniform for uniform.
"""
import numpy as np
import pandas as pd
import torch
...
out_fname = f"results/wor_sl={seed_chunk}.csv"
...
def run_wor_batched(Y_t, Yhat_t, N_B, active, rng_seed):
    ...
    Yhat_t: (N,) tensor — model predictions (fixed)
    N_B: budget
    active: bool — if True, use min(Yhat, 1-Yhat) scoring; else uniform
    ...
    for t in range(N_B):
        ...
        # Imputed sum: sum_{i in O} y_i + sum_{i not in O} f_i (without 1/N factor)
        imputed_sum = (
        ...
        # B_hat accumulator: for t >= 2, i.e., t >= 1 in 0-indexed
        if t >= 1:
