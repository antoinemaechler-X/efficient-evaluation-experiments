"""
Plot Pew ATP Wave 79 study results.

Produces PDFs:
  1. Main: WOR Active vs Active Inference vs Classical (ESS + coverage)
...
method_config = [
    ...
    ("active-inference",   "P", colors[5], "Active Inference (Zrnic)", True),
    ("classical",          "x", colors[4], "Classical",                 False),
]

groups_config = [
    ("all", "Pew ATP Wave 79 - Biden Approval Rate", 0),
]
...
def make_figure(ess_col, ess_serr_col, methods, out_path, legend_ncol=3, budget_subset=None):
    ...
    """
    # Only show methods that actually have results (WOR and active inference are pending).
...
if "wor-uniform" in summary["method"].unique():
    make_figure("ess_multiplier", "ess_multiplier_serr",
                method_config,
                "results/pew_ess+coverage_vs_wor_uniform.pdf")

# Plot 2: ESS relative to Classical
make_figure("ess_multiplier_vs_classical", "ess_multiplier_vs_classical_serr",
            method_config,
            "results/pew_ess+coverage_vs_classical.pdf")

# Plot 3: WOR Active vs Active Inference vs Classical, ESS relative to Classical
method_config_main = [
    ("wor-active",         "o", colors[0], "WOR Active (Ours)",        True),
    ("active-inference",   "P", colors[5], "Active Inference (Zrnic)", True),
