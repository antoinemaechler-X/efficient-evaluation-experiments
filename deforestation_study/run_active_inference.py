"""
Active Inference estimator for Amazon deforestation dataset.

Usage: python deforestation_study/run_active_inference.py <seed_chunk>
  seed_chunk in {0, 1, 2}: splits 1000 seeds into 3 chunks.

Reproduces the Active Inference method from Zrnic & Candes (2024) EXACTLY:
  https://github.com/tijana-zrnic/active-inference

Implementation matches their pew79 notebook cell-by-cell:
  - Tau tuned per budget on train2 using separate tuning model predictions
  - Variance proxy: mean((Y_train2 - Yhat_train2)^2 / pi(tau))
  - Tau grid: linspace(0.01, 0.99, 100) (same as paper)
  ...
  - Coverage checked against theta_true = mean(Y_all) (all data, not just test)

Model: HistGradientBoostingClassifier(max_iter=100, max_depth=2) — same as
       Zrnic's cross-ppi deforestation experiment for fair comparison.
"""
import numpy as np
import pandas as pd
import torch
...
# Tau tuning grid: exactly 100 values as in Zrnic's notebook
...
# Training data for tau tuning (from separate tuning model, as in Zrnic)
Y_train2 = np.load(f"{data_dir}/Y_train2.npy").flatten().astype(np.float32)
Yhat_train2 = np.load(f"{data_dir}/Yhat_train2.npy").flatten().astype(np.float32)

# theta_true = mean(Y_all), matching Zrnic exactly
theta_true = float(np.load(f"{data_dir}/theta_true.npy")[0])

N_total = len(Y)
print(f"Test set N={N_total}, theta_true={theta_true:.6f} (from all data)")
...
logs_fname = f"deforestation_study/logs/active_inference_sl={seed_chunk}.csv"
...
def tune_tau(Y_t2, Yhat_t2, budget_prop):
    """
    Tune tau by minimizing empirical variance on train2.

    Exactly as in Zrnic's notebook:
      eta = budget / mean(uncertainty_train)
      for tau in taus:
          probs = clip((1-tau)*eta*uncertainty_train + budget*tau, 0, 1)
          var_tau = mean((y_train2 - Yhat_train)**2 / probs)
      return tau with min var

    Returns: optimal tau value
    """
    uncertainty_train = np.minimum(Yhat_t2, 1.0 - Yhat_t2)
    residuals_sq = (Y_t2 - Yhat_t2) ** 2
    eta = budget_prop / np.mean(uncertainty_train)

    best_var = np.inf
    best_tau = 0.5

    for tau in TAU_GRID:
        probs = np.clip((1.0 - tau) * eta * uncertainty_train + budget_prop * tau, 0, 1)
...
def run_active_inference(Y_group, Yhat_group, budget_prop, tuned_tau, rng_seed):
    ...
    Exactly reproduces Zrnic & Candes (2024):
      eta = budget / mean(uncertainty)
      probs = clip((1-tau)*eta*uncertainty + tau*budget, 0, 1)
      xi ~ Bernoulli(probs)
      increments = Yhat + (Y - Yhat) * xi / probs
      theta_hat = mean(increments)
      se = std(increments) / sqrt(n)
      CI = theta_hat +/- z * se
    ...
    # Compute active sampling probabilities (same formula as Zrnic)
    ...
    # Move to torch for vectorized Bernoulli sampling
    ...
    # AIPW increments: Yhat + (Y - Yhat) * xi / pi  (exactly as Zrnic)
    increments = Yhat_t + (Y_t - Yhat_t) * xi / pi_t  # (N_BATCH, N)

    # Point estimate: mean of increments
    theta_hat = increments.mean(dim=1)  # (N_BATCH,)

    # Standard error: std(increments) / sqrt(n)  (exactly as Zrnic)
    # Zrnic uses np.std (ddof=0), so we use correction=0 to match
    se = increments.std(dim=1, correction=0) / np.sqrt(N)  # (N_BATCH,)

    # CI: theta_hat +/- z * se (NO clipping, exactly as Zrnic)
    halfwidth = z_score * se
    ub = theta_hat + halfwidth
    lb = theta_hat - halfwidth

    # Full interval width = 2 * halfwidth (as recorded by Zrnic: "2*width_active")
    widths = (2 * halfwidth).cpu().numpy()

    # Coverage against theta_true (mean of ALL data, as in Zrnic)
...
for group_idx, (group_name, group_mask) in enumerate(groups.items()):
    ...
    for budget_idx, budget_prop in enumerate(BUDGET_PROPS):
        ...
        if counter >= checkpoint_counter:
            # Tune tau on training data (same for all seeds at this budget)
            tuned_tau = tune_tau(Y_train2, Yhat_train2, budget_prop)
