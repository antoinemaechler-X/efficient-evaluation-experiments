"""
WOR FAQ Validation: Tune hyperparameters (beta0, rho, gamma, tau) on M1 train/val split.
Without-replacement version of faq_val.py.

Only fully-observed historical data (n_full_obs=None, mcar_obs_prob=1.0).
Budgets: 10 evenly-spaced values between 2.5% and 25%.

Usage:
    python wor_faq_val.py <dataset_idx> <gamma_idx>
    dataset_idx: 0 = mmlu-pro, 1 = bbh+gpqa+ifeval+math+musr
    gamma_idx: 0-4 (indexes into GAMMA_VALS)
"""
import numpy as np
import pandas as pd
import torch
