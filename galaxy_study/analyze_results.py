"""
Analyze Galaxy Zoo 2 study results.

Loads WOR, Bernoulli, and Cross-PPI CSV files, computes ESS multipliers
relative to WOR-uniform and Classical, and builds a summary table.

Output: galaxy_study/logs/cleaned/summary.csv
"""
import numpy as np
import pandas as pd
import os
...
# ====================================================================
wor_files = glob.glob("galaxy_study/logs/wor_sl=*.csv")
bernoulli_files = glob.glob("galaxy_study/logs/bernoulli_sl=*.csv")
cross_ppi_files = glob.glob("galaxy_study/logs/cross_ppi_sl=*.csv")
...
all_df = pd.concat([wor_df, bern_df, cppi_df], ignore_index=True)
...
# PART 4: Save
# ====================================================================
os.makedirs("galaxy_study/logs/cleaned", exist_ok=True)

all_df.to_csv("galaxy_study/logs/cleaned/all_per_seed.csv", index=False)
summary.to_csv("galaxy_study/logs/cleaned/summary.csv", index=False)

print(f"\nSaved per-seed results: galaxy_study/logs/cleaned/all_per_seed.csv")
print(f"Saved summary: galaxy_study/logs/cleaned/summary.csv")
