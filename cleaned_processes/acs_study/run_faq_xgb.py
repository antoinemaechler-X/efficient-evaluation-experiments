...
XGBoost is trained on the FULL labeled split for best possible AIPW variance reduction.
BLR is fitted on --n_labeled_blr points only to keep genuine posterior uncertainty
...
def trial(
    ...
    D = V.shape[1]
    R = Y - Y_XGB                                                         # (N_Q,) residuals after XGB
    ...
    for s in tqdm(range(N_B), desc=f"{str(counter).zfill(5)}", disable=not sys.stderr.isatty()):
        ...
        # e. exploration + tempering governors
        ...
        y_Is    = torch.gather(Y.unsqueeze(0).expand(N_NEW, -1), 1, I_s)
        yhat_Is = torch.gather(y_hat_js, 1, I_s)
        ...
        #### PART 3: BLR Posterior Update on Residuals (Sherman-Morrison)
        ...
        r_Is    = torch.gather(R.unsqueeze(0).expand(N_NEW, -1), 1, I_s)
        rhat_Is = torch.gather(r_hat_js, 1, I_s)
        Uhats  += torch.bmm(
            Sigmahats,
...
def main():
    ...
    p.add_argument('--n_labeled_blr',   type=int,   default=500,
                   help="Points used to initialise the BLR prior on XGB residuals.")
    ...
    # train XGBoost on the full labeled split
    print(f"Training XGBoost ({args.xgb_rounds} rounds) on {n_tr:,} points...")
    ...
    print(f"XGBoost R² labeled={r2_score(y_lab_n, yhat_xgb_lab):.3f}  "
          f"unlabeled={r2_score(y_unlab_n, yhat_xgb_unlab):.3f}")

    # fit BLR on residuals with n_labeled_blr points only
    n_blr = min(args.n_labeled_blr, n_tr)
    print(f"Fitting BLR on residuals ({n_blr:,} points, D={args.D})...")
    r_lab = (y_lab_n - yhat_xgb_lab).astype(np.float64)
    mu_post, Sig_post, sigma2 = fit_blr(
        V_lab_np[:n_blr].astype(np.float64), r_lab[:n_blr], prior_tau=args.prior_tau,
    )
